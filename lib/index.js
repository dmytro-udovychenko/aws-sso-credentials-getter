...
const getConfigForProfile = (config, profile) => {
    ...
    if (
        ...
        typeof profileConfig.sso_session !== "undefined"
    ) {
        const ssoSessionKey = "sso-session " + profileConfig.sso_session
        if (typeof config[ssoSessionKey].sso_region !== "undefined") {
            profileConfig.sso_region = config[ssoSessionKey].sso_region
        }
    }

    if (typeof profileConfig.sso_region === "undefined") {
        console.error("The profile is missing the sso_region field")
        return undefined
    }
